...
local admins = { 5225356162, 1360277717 }
...
local function obey_command(Player, Message)
	...
	exec_cmd({
		...
		callback = function()
			...
			commands.corner = false
		end
	})

	exec_cmd({
		prompt = { "$disable wow all", "$disable wow " .. local_player.Name },
		callback = function()
			flags.walk_on_water = false
			commands.water = false
		end
	})

	exec_cmd({
		prompt = { "$enable wow all", "$enable wow " .. local_player.Name },
		callback = function()
			flags.walk_on_water = true
			commands.water = true
		end
	})
